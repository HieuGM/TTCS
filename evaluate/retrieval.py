...
from src.retrieval_pipeline import retrieve_rrf_candidates


DEFAULT_GROUND_TRUTH = REPO_ROOT / "ground_truth" / "grounth_truth_record_id.csv"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "evaluate" / "logs"
...
def load_questions(path: Path) -> List[Dict[str, str]]:
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row_num, row in enumerate(reader, start=2):
            ...
            items.append({"id": row_id, "question": question})
...
def chunk_payload(doc: Any) -> Dict[str, str]:
...
def export_retrieved_chunks(
    ...
    *,
    ...
    for index, item in enumerate(questions, start=1):
        ...
        docs, _ = retrieve_rrf_candidates(item["question"], DEFAULT_RETRIEVAL_CONFIG)
        records.append(
            {
                "id": item["id"],
